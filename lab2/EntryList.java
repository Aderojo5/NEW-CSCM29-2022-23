...
public class EntryList{
     ...
     *
     *   done by first converting the current EntryList*/

public class EntryList {
    private List<TxEntry> list;

    /** 
     * Constructor for EntryList with no entries
     */
    public EntryList() {
        list = new ArrayList<TxEntry>();
    }

    /** 
     * Constructor for EntryList with one entry
     */
    public EntryList(String user, int amount) {
        this();
        add(user, amount);
    ...
    /** 
     * Constructor for EntryList with two entries
     */
    public EntryList(String user1, int amount1, String user2, int amount2) {
        this(user1, amount1);
        add(user2, amount2);
    ...
    /** 
     * add(String user, int amount) 
     * adds a new entry to the list
     */
    public void add(String user, int amount) {
        list.add(new TxEntry(user, amount));
    }

    /** 
     * toSum() 
     * returns the sum of amounts in the list
     */
    public int toSum() {
        int sum = 0;
        for (TxEntry entry : list) {
            sum += entry.getAmount();
        }
        return sum;
    }
    ...
    /** 
     * print() 
     * prints out the entries in the list
     */
    public void print() {
        print("", "");
    }
    ...
    /** 
     * print(String userPrefix, String amountPrefix) 
     * prints out the entries in the list with the given prefixes
     */
    public void print(String userPrefix, String amountPrefix) {
        for (TxEntry entry : list) {
            System.out.println(userPrefix + entry.getUser() + amountPrefix + entry.getAmount());
        }
    }
    ...
    public void subtractFromLedger(Ledger ledger){
        for (TxEntry entry : list) {
            ledger.subtractFromUser(entry.getUser(), entry.getAmount());
        }
    }
    ...
    public void addToLedger(Ledger ledger){
        for (TxEntry entry : list) {
            ledger.addToUser(entry.getUser(), entry.getAmount());
        }
    }
